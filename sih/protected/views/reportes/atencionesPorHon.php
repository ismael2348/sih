...
<?php
if($anioy != 0){
    $info = $connection->createCommand("SELECT sum(montoHonorarios) as total, MONTH(fechaCreacion) as aseg FROM pacientes_aseguradora WHERE YEAR(fechaCreacion) = ".$anioy." AND montoHonorarios != '' AND montoHonorarios > 0  GROUP BY MONTH(fechaCreacion) ORDER BY MONTH(fechaCreacion) ASC")->queryAll();
/*
echo "<pre>";
    print_r($info);
echo "</pre>";*/
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'Monto de honorarios por mes durante el <?php echo $anioy; ?>'
        },
         subtitle: {
            text: 'Hospital siloé'
        },
        ...
        tooltip: {
            pointFormat: '$<b>{point.y}</b> pesos en honorarios.</b>'
        },
        series: [{
