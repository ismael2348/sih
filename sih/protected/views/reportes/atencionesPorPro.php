...
<?php
 //echo "su anio ".$anioy." y su mes ".$mesm;
 	...
 	$info = $connection->createCommand("SELECT count(Proveedor1) as  total, Proveedor1 as aseg FROM pacientes_aseguradora WHERE MONTH(fechaCreacion) = ".$mesm." AND YEAR(fechaCreacion) = ".$anioy." AND Proveedor1 != '' GROUP BY Proveedor1 ORDER BY total DESC")->queryAll();
...
foreach ($info as $key => $value) {
	...
	$valores .="['".$value["aseg"]."',".$value["total"]."],";
}
$valores.="";
/*
$valores = "";
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'Proveedores principales por atenciones durante el mes de <?php echo $mesesx[$mesm-1]." del ".$anioy; ?>'
        },
         subtitle: {
            text: 'Hospital siloé'
        },
...
<?php
	...
	echo "No hay resultados disponibles que concuerden con su busqueda.<br>intente parametros diferentes tanto en año y mes.";
}
?>