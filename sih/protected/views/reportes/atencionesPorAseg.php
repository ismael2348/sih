...
<?php
 //echo "su anio ".$anioy." y su mes ".$mesm;
    ...
    $info = $connection->createCommand("SELECT count(pa.id_aseguradora) as  total, a.nombre as aseg FROM pacientes_aseguradora AS pa JOIN aseguradoras AS a ON a.id = pa.id_aseguradora WHERE MONTH(pa.fechaCreacion) = ".$mesm." AND YEAR(pa.fechaCreacion) = ".$anioy."  GROUP BY pa.id_aseguradora ORDER BY total DESC")->queryAll();
...
foreach ($info as $key => $value) {
    ...
    $valores .="['".substr($value["aseg"], 0, $mitad)."<br>".substr($value["aseg"], $mitad)."',".$value["total"]."],";
}
$valores.="";
/*
$valores = "";
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'Pacientes tratados por aseguradora el mes <?php echo $mesesx[$mesm-1]." del ".$anioy; ?>'
        },
         subtitle: {
            text: 'Hospital siloé'
        },
        ...
        tooltip: {
            pointFormat: 'Population in 2008: <b>{point.y} pacientes</b>'
        },
        series: [{
