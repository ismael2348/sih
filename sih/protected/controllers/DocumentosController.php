...
class DocumentosController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Documentos']))
		{
			...
			$ruta = "sih" . "/" . "sih" . "/documentos/";
			$nombredoc = $_POST['Documentos']['nombre'];
			if (!file_exists($ruta)) {
				mkdir($ruta, 0777, true);
			}

			if($model->url = CUploadedFile::getInstance($model,'url')){
				    $model->url->saveAs($ruta . $nombredoc . '.' . $model->url->getExtensionName());
					$model->url = $ruta . $nombredoc . '.' .$model->url->getExtensionName();
